...
from .const import DOMAIN, WATERING_CATEGORIES

_LOGGER = logging.getLogger(__name__)

# Weekday names indexed 0=Sun … 6=Sat (Tuya bitmask order)
_TUYA_DAYS = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]


# ---------------------------------------------------------------------------
# Schedule binary decoding
# ---------------------------------------------------------------------------

def _decode_entry_ggq(data: bytes) -> dict | None:
    """Decode a 13-byte ggq schedule entry."""
    if len(data) < 13:
        return None
    start_mins = struct.unpack(">H", data[0:2])[0]
    duration_mins = struct.unpack(">H", data[2:4])[0]
    ...
    zone_index = data[12]
    return _build_entry(
        start_mins, duration_mins, days_bitmask, volume_pct,
        flags, year, month, day, interval, zone_index,
    )


def _decode_entry_sfkzq(data: bytes) -> dict | None:
    """Decode a 12-byte sfkzq schedule entry."""
    if len(data) < 12:
        return None
    start_mins = struct.unpack(">H", data[0:2])[0]
    duration_mins = struct.unpack(">H", data[2:4])[0]
...
def _decode_timer_payload(b64_value: str, category: str) -> list[dict]:
    """Decode a base64 schedule payload into a list of schedule entries."""
    try:
        raw = base64.b64decode(b64_value)
    except Exception:
        return []
    ...
    entries = []
    if category == "sfkzq":
        # 2-byte header + 12-byte entry
        if len(raw) >= 14:
            e = _decode_entry_sfkzq(raw[2:14])
            if e:
                entries.append(e)
    elif category == "ggq":
        # 2-byte header then 13-byte entries separated by 1-byte delimiters
        pos = 2
        while pos + 13 <= len(raw):
            e = _decode_entry_ggq(raw[pos:pos + 13])
            if e:
                entries.append(e)
            pos += 13
            # Skip the 1-byte separator between entries (not after the last one)
            if pos < len(raw) and pos + 13 <= len(raw):
                pos += 1
...
# ---------------------------------------------------------------------------
# Next scheduled time computation
# ---------------------------------------------------------------------------

def _compute_next_scheduled(entry: dict) -> datetime | None:
    """Return the next datetime this schedule entry will run, or None."""
    if not entry or not entry.get("enabled"):
        return None
    ...
    if mode == "weekly":
        ...
        for ahead in range(35):
            check = today + timedelta(days=ahead)
            if (cal_type == "even" and check.day % 2 == 0) or (cal_type == "odd" and check.day % 2 == 1):
                dt = datetime(check.year, check.month, check.day, start_h, start_m)
                if dt > now:
                    return dt
        return None
...
def _schedule_summary(entry: dict | None) -> str | None:
    """One-line human-readable schedule description."""
    if not entry:
        return None
    start_mins = entry.get("start_minutes", 0)
    start_h, start_m = divmod(start_mins, 60)
...
class TuyaGardenCoordinator(DataUpdateCoordinator):
    """Fetch and decode all garden watering timer data from Tuya Cloud."""
    ...
    def __init__(self, hass: HomeAssistant, config: dict) -> None:
        super().__init__(
            ...
            _LOGGER,
            name=DOMAIN,
            update_interval=timedelta(seconds=config.get(DOMAIN + "_scan_interval", config.get("scan_interval", 300))),
        )
        self._config = config
        self._cloud: tinytuya.Cloud | None = None
    ...
    def _fetch_all(self) -> dict:
        ...
        # getdevices() must be called before shadow queries (initialises auth state)
        devices = cloud.getdevices()
        if not isinstance(devices, list):
            raise UpdateFailed("Tuya API returned no device list")
        ...
        for dev in watering:
            ...
            category: str = dev.get("category", "ggq")
            name: str = dev.get("name", device_id)

            try:
                props = self._fetch_shadow(cloud, device_id)
            except Exception as err:
                _LOGGER.warning("Shadow fetch failed for %s (%s): %s", name, device_id, err)
                props = {}

            battery = props.get("battery_percentage", {}).get("value")

            zones = self._build_zones(props, category)
    ...
    def _build_zones(self, props: dict, category: str) -> list[dict]:
        zones: list[dict] = []

        if category == "ggq":
            schedule_b64 = props.get("normal_timer", {}).get("value")
            entries = _decode_timer_payload(schedule_b64, "ggq") if schedule_b64 else []

            zone_defs = [
                (1, "switch_1", "weather_delay",  "use_time_1"),
                (2, "switch_2", "weather_delay2", "use_time_2"),
            ]
            for zone_num, switch_code, wd_code, ut_code in zone_defs:
                # Custom name stored on the switch property
                zone_name = (
                    props.get(switch_code, {}).get("custom_name")
                    or f"Zone {zone_num}"
                )
                zone_entries = [e for e in entries if e.get("zone") == zone_num]
                active = zone_entries[0] if zone_entries else None

                ut_prop = props.get(ut_code, {})
                rd_prop = props.get(wd_code, {})
                sw_prop = props.get(switch_code, {})

                zones.append({
                    "zone_num": zone_num,
                    "name": zone_name,
                    "switch_code": switch_code,
                    "rain_delay_code": wd_code,
                    "valve_open": bool(sw_prop.get("value", False)),
                    "last_watered_duration_s": ut_prop.get("value"),
                    "last_watered_at_ms": ut_prop.get("time"),
                    "rain_delay": str(rd_prop.get("value", "OFF")) if rd_prop.get("value") is not None else "OFF",
                    "schedule_entry": active,
                    "next_scheduled": _compute_next_scheduled(active),
                    "schedule_summary": _schedule_summary(active),
                })
            ...
            schedule_b64 = props.get("timer", {}).get("value")
            entries = _decode_timer_payload(schedule_b64, "sfkzq") if schedule_b64 else []
            active = entries[0] if entries else None

            zone_name = props.get("switch", {}).get("custom_name") or "Zone 1"
            ut_prop = props.get("use_time_1", {})
            rd_prop = props.get("weather_delay", {})
            sw_prop = props.get("switch", {})
            ...
            zones.append({
                ...
                "last_watered_duration_s": ut_prop.get("value"),
                "last_watered_at_ms": ut_prop.get("time"),
                "rain_delay": str(rd_prop.get("value", "cancel")) if rd_prop.get("value") is not None else "cancel",
                "schedule_entry": active,
                "next_scheduled": _compute_next_scheduled(active),
                "schedule_summary": _schedule_summary(active),
            })
    ...
    async def _async_update_data(self) -> dict:
        try:
            ...
            raise UpdateFailed(f"Error fetching Tuya data: {err}") from err

    def send_command(self, device_id: str, commands: list[dict]) -> bool:
        """Send cloud commands to a device (blocking — call via executor)."""
        try:
            cloud = self._get_cloud()
            result = cloud._tuyaplatform(
                f"devices/{device_id}/commands",
            ...
            if not success:
                _LOGGER.warning(
                    "Command to %s returned non-success: %s",
                    device_id, result,
                )
            return bool(success)
        except Exception as err:
